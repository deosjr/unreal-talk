...
; muKanren), creates the one datalog instance the whole engine wraps, and
; then loads the engine/ modules that build the RealTalk layer on top.
...
;   * fixpoint.scm overrides datalog.scm's dl-fixpoint!, so it must come
;     after datalog.scm (last define wins).
...
(include "engine/dsl.scm")      ; Claim/Wish/When/Collect macros + *rule-procs*
(include "engine/fixpoint.scm") ; dl-fixpoint! override (runs rule bodies)
